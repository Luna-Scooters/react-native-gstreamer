...
static const guint64 qos_count_max = 50; // Reduced from 100 to respond faster to quality issues
static guint refresh_qos_count_ms = 5000; // Reduced from 10 seconds to 5 seconds
...
static void cb_source_created(GstElement *pipe, GstElement *source) {
    g_object_set(source,
                "latency", 50, /* Reduced from 150 ms to 50 ms */
                "buffer-mode", 0,
                "drop-on-latency", TRUE,
                "ntp-sync", FALSE,
                "max-ts-offset", 30 * 1000 * 1000, /* Reduced from 50 ms to 30 ms */
                "protocols", 0x03, /* UDP + UDP_MCAST */
                "udp-buffer-size", 2097152, /* Reduced from 5 MB to 2 MB */
                "max-rtcp-rtp-time-diff", 100 * 1000 * 1000, /* Reduced from 200 ms to 100 ms */
...
void rct_gst_init(RctGstConfiguration *configuration)
{
    ...
    // Always use fakesink for audio to completely ignore it
    // Add latency configuration to playbin for low latency
